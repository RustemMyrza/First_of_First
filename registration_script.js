...
signform.addEventListener("submit", (e) =>{
    let signupdata = {
        ...
        if(signupdata.nickname==""){
            document.getElementById("outnickname").innerHTML = "Please fill in this field"
        }
        else if(signupdata.nickname.length >= 30){
            document.getElementById("outnickname").innerHTML = "Your nickname must be shorter than 30 symbols"
        }
        else if(signupdata.nickname.length < 1){
            document.getElementById("outnickname").innerHTML = "Your nickname must be longer than 1 symbols";
        }
        else{
            document.getElementById("outnickname").innerHTML = "Accepted";
            document.getElementById("outnickname").style.color = "green";
            correctforms++;
        }
        ...
        if(correctforms == 7){
            let database = JSON.parse(localStorage.getItem("signupdata")) || [];
            if(!(database instanceof Array)){
                database = [database];
            }
            database.push(signupdata);
            localStorage.setItem("signupdata", JSON.stringify(database));
